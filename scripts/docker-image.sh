...
FELIB=${SCRIPT_DIR}/../fe-config/flogo.zip
...
services:
  dovetail:
    ...
    environment:
      ...
      - WORK=/root/work
      - DOVETAIL_REPO=github.com/dovetail-lab
...
function buildImage {
  ...
  # this scripts runs in container to initialize the image, so copy it to shared volume
...
function printHelp() {
  ...
  echo "    -v <image version> - version of the docker image, e.g., 'v1.2.0' (default)"
  echo "    -e <flogo lib> - path of the zip file for Flogo Enterprise library, e.g., '../fe-config/flogo.zip' (default)"
